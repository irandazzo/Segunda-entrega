...
products.get("/:id", async (req, res) => {
    ...
    if (!result) {
    return res.status(200).send(`No existe el producto con el Id ${id}`);
    }
...
products.post("/", async (req, res) => {
    ...
    return res.status(200).send(`Complete todos los campos para crear el producto`);
    }
...
products.put("/:id", async (req, res) => {
    ...
    if (!title || !description || !code || !price || !stock || !category || !price) {
    return res.status(200).send(`Complete todos los campos para actualizar el producto`);
    }
